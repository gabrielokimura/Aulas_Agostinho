...
def login ():
    ...
    if request.method == "POST":
        ...
        tamanho2 = len(modelo.hospedes)
        for i in range (tamanho2):
            if modelo.hospedes[i]["nome"] == nome and modelo.hospedes[i]["cpf"] == senha:
                session["hospede"] = modelo.hospedes[i]
...
def hospedes():
    hospede = session.get("hospede")
    if not hospede:
        return redirect(url_for("login"))
    return render_template("hospedes.html", hospede = hospede, modelo = modelo)
